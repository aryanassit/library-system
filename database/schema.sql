...
('New book added: The Great Gatsby by F. Scott Fitzgerald', 2),
('New user registered: John Doe (john.doe@example.com)', 2),
('Book "1984" borrowed', 1),
('System backup completed', NULL),
('New user registered: Jane Smith (jane.smith@example.com)', 2);
...
CREATE TRIGGER update_books_updated_at AFTER UPDATE ON books
BEGIN
...
CREATE TRIGGER limit_activities AFTER INSERT ON activities
BEGIN
    DELETE FROM activities WHERE id NOT IN (
        SELECT id FROM activities ORDER BY timestamp DESC LIMIT 50
    );
END;
